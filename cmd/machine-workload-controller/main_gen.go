...
import (
	...
	machineworkload "github.com/threeport/threeport/internal/machine-workload"
...
func main() {
	// flags
	var machineWorkloadInstanceConcurrentReconciles = flag.Int(
		"machine-workload-instance-concurrent-reconciles",
		1,
		"Number of concurrent reconcilers to run for machine workload instances",
	)
	...
	kvConfig := natsgo.KeyValueConfig{
		Bucket:      machineworkload.LockBucketName,
		Description: machineworkload.LockBucketDescr,
		TTL:         time.Minute * 20,
	}
	kv, err := controller.CreateLockBucketIfNotExists(js, &kvConfig)
	if err != nil {
		log.Error(err, "failed to bind to JetStream key-value locking bucket", "lockBucketName", machineworkload.LockBucketName)
		os.Exit(1)
	}
	...
	reconcilerConfigs = append(reconcilerConfigs, controller.ReconcilerConfig{
		ConcurrentReconciles: *machineWorkloadInstanceConcurrentReconciles,
		Name:                 "MachineWorkloadInstanceReconciler",
		NotifSubject:         notif.MachineWorkloadInstanceSubject,
	...
	for _, r := range reconcilerConfigs {
		...
		consumerInfo, err := notifications.EnsureConsumer(js, notif.MachineWorkloadStreamName, consumerConfig)
		if err != nil {
		...
		reconciler := controller.Reconciler{
			...
			EventsRecorder: &event.EventRecorder{
				APIClient:           apiClient,
				APIServer:           *apiServer,
				ReportingController: "MachineWorkloadController",
			},
			JetStreamContext: js,
			KeyValue:         kv,
			Log:              &log,
	...
	log.Info("machine-workload controller shutting down")
	os.Exit(0)
}
func showHelpAndExit(exitCode int) {
	fmt.Printf("Usage: threeport-machine-workload-controller [options]\n")
	fmt.Println("options:")
	flag.PrintDefaults()
	os.Exit(exitCode)
}
