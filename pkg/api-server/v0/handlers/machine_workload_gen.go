...
func (h Handler) AddMachineWorkloadDefinition(c echo.Context) error {
	...
	if result := apiserver_lib.RetryOnSerializationFailure(func() *gorm.DB {
		return h.RequestDB(c).Create(&machineWorkloadDefinition)
	}); result.Error != nil {
...
func (h Handler) GetMachineWorkloadDefinitions(c echo.Context) error {
	...
	case pageParams.QueryId == "":
		...
		case true:
			...
			queryId, count, err := h.DispatchGetPaginatedRecords(h.PaginationMode, h.RequestDB(c).Model(&api_v0.MachineWorkloadDefinition{}).Where(&filter), records, queryTable, pageParams)
			if err != nil {
...
func (h Handler) UpdateMachineWorkloadDefinition(c echo.Context) error {
	...
	if result := apiserver_lib.RetryOnSerializationFailure(func() *gorm.DB {
		return h.RequestDB(c).Model(&existingMachineWorkloadDefinition).Updates(&updatedMachineWorkloadDefinition)
	}); result.Error != nil {
...
func (h Handler) ReplaceMachineWorkloadDefinition(c echo.Context) error {
	...
	if result := apiserver_lib.RetryOnSerializationFailure(func() *gorm.DB {
		return h.RequestDB(c).Session(&gorm.Session{FullSaveAssociations: false}).Omit("CreatedAt", "DeletedAt").Save(&updatedMachineWorkloadDefinition)
	}); result.Error != nil {
...
func (h Handler) DeleteMachineWorkloadDefinition(c echo.Context) error {
	...
	if len(machineWorkloadDefinition.MachineWorkloadInstances) != 0 {
		blockingChildren := make([]api_lib.FullyQualifiedTypeProvider, 0, len(machineWorkloadDefinition.MachineWorkloadInstances))
		for i := range machineWorkloadDefinition.MachineWorkloadInstances {
			blockingChildren = append(blockingChildren, machineWorkloadDefinition.MachineWorkloadInstances[i])
		}
		return RespondBlockedDelete(
			c,
			h.RequestDB(c),
			api_v0.NewBlockedDeleteErrorFromChildren(&machineWorkloadDefinition, blockingChildren),
		)
	}

	// delete object
	if result := apiserver_lib.RetryOnSerializationFailure(func() *gorm.DB {
		return h.RequestDB(c).Delete(&machineWorkloadDefinition)
	}); result.Error != nil {
...
func (h Handler) AddMachineWorkloadInstance(c echo.Context) error {
	...
	// persist to DB
	if result := apiserver_lib.RetryOnSerializationFailure(func() *gorm.DB {
		return h.RequestDB(c).Create(&machineWorkloadInstance)
	}); result.Error != nil {
		h.Logger.Error("handler error: error creating object", zap.Error(result.Error))
		// check if this is a custom HTTP error with specific status code
		var httpErr *util_v0.HttpError
...
func (h Handler) GetMachineWorkloadInstances(c echo.Context) error {
	...
	case pageParams.QueryId == "":
		...
		case true:
			// dispatch to the configured pagination strategy to fetch the first page
			queryTable := filter.TableName()
			queryId, count, err := h.DispatchGetPaginatedRecords(h.PaginationMode, h.RequestDB(c).Model(&api_v0.MachineWorkloadInstance{}).Where(&filter), records, queryTable, pageParams)
			if err != nil {
				h.Logger.Error("handler error: error fetching paginated records", zap.Error(err))
				return apiserver_lib.ResponseStatus500(c, pageParams, err, objectType)
			}
...
func (h Handler) ReplaceMachineWorkloadInstance(c echo.Context) error {
	...
	if result := apiserver_lib.RetryOnSerializationFailure(func() *gorm.DB {
		return h.RequestDB(c).Session(&gorm.Session{FullSaveAssociations: false}).Omit("CreatedAt", "DeletedAt").Save(&updatedMachineWorkloadInstance)
	}); result.Error != nil {
		h.Logger.Error("handler error: error persisting object", zap.Error(result.Error))
		// check if this is a custom HTTP error with specific status code
		var httpErr *util_v0.HttpError
...
func (h Handler) DeleteMachineWorkloadInstance(c echo.Context) error {
	...
	if machineWorkloadInstance.DeletionScheduled == nil {
		...
		if result := apiserver_lib.RetryOnSerializationFailure(func() *gorm.DB {
			return h.RequestDB(c).Model(&machineWorkloadInstance).Updates(&scheduledMachineWorkloadInstance)
		}); result.Error != nil {
		...
		if machineWorkloadInstance.DeletionConfirmed == nil {
			...
			// from DB
			if result := apiserver_lib.RetryOnSerializationFailure(func() *gorm.DB {
				return h.RequestDB(c).Delete(&machineWorkloadInstance)
			}); result.Error != nil {
				h.Logger.Error("handler error: error deleting object", zap.Error(result.Error))
