...
import (
	...
	controlplane "github.com/threeport/threeport/internal/control-plane"
...
func main() {
	...
	kvConfig := natsgo.KeyValueConfig{
		Bucket:      controlplane.LockBucketName,
		Description: controlplane.LockBucketDescr,
		TTL:         time.Minute * 20,
	}
	kv, err := controller.CreateLockBucketIfNotExists(js, &kvConfig)
	if err != nil {
		log.Error(err, "failed to bind to JetStream key-value locking bucket", "lockBucketName", controlplane.LockBucketName)
		os.Exit(1)
	}
	...
	for _, r := range reconcilerConfigs {
		...
		consumerInfo, err := notifications.EnsureConsumer(js, notif.ControlPlaneStreamName, consumerConfig)
		if err != nil {
		...
		sub, err := js.PullSubscribe(r.NotifSubject, consumer, natsgo.BindStream(notif.ControlPlaneStreamName))
		if err != nil {
	...
	log.Info(
		"control-plane controller started",
		"version", version.GetVersion(),
		"controllerID", controllerID.String(),
		"NATSConnection", natsConn,
		"lockBucketName", controlplane.LockBucketName,
	)
	...
	log.Info("control-plane controller shutting down")
	os.Exit(0)
}
func showHelpAndExit(exitCode int) {
	fmt.Printf("Usage: threeport-control-plane-controller [options]\n")
	fmt.Println("options:")
	flag.PrintDefaults()
	os.Exit(exitCode)
}
