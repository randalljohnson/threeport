...
const helperEnvVar = "HELM_WORKLOAD_CTRL_HELP_EXIT_CODE"
...
const mainHelperEnvVar = "HELM_WORKLOAD_CTRL_RUN_MAIN"
...
func TestMain(m *testing.M) {
	...
	if argv, ok := os.LookupEnv(mainHelperEnvVar); ok {
		...
		os.Args = append([]string{"helm-workload-controller"}, strings.Fields(argv)...)
		main()
...
func TestShowHelpAndExitPrintsUsage(t *testing.T) {
	...
	wantPrefix := "Usage: threeport-helm-workload-controller [options]"
...
func TestMainHelpFlagPrintsFlagDefaults(t *testing.T) {
	...
	if !strings.HasPrefix(stdout, "Usage: threeport-helm-workload-controller [options]") {
	...
	// verify a concurrency flag also shows, confirming reconciler flags register.
	if !strings.Contains(stderr, "-helm-workload-definition-concurrent-reconciles") {
		t.Fatalf("stderr missing -helm-workload-definition-concurrent-reconciles flag: %q", stderr)
	}
	// verify the instance concurrency flag shows so both reconciler flags register.
	if !strings.Contains(stderr, "-helm-workload-instance-concurrent-reconciles") {
		t.Fatalf("stderr missing -helm-workload-instance-concurrent-reconciles flag: %q", stderr)
	}
