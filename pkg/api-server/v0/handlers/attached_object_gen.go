...
func (h Handler) AddAttachedObjectReference(c echo.Context) error {
	...
	if result := apiserver_lib.RetryOnSerializationFailure(func() *gorm.DB {
		return h.RequestDB(c).Create(&attachedObjectReference)
	}); result.Error != nil {
...
func (h Handler) GetAttachedObjectReferences(c echo.Context) error {
	...
	case pageParams.QueryId == "":
		...
		case true:
			...
			queryId, count, err := h.DispatchGetPaginatedRecords(h.PaginationMode, h.RequestDB(c).Model(&api_v0.AttachedObjectReference{}).Where(&filter), records, queryTable, pageParams)
			if err != nil {
...
func (h Handler) UpdateAttachedObjectReference(c echo.Context) error {
	...
	if result := apiserver_lib.RetryOnSerializationFailure(func() *gorm.DB {
		return h.RequestDB(c).Model(&existingAttachedObjectReference).Updates(&updatedAttachedObjectReference)
	}); result.Error != nil {
...
func (h Handler) ReplaceAttachedObjectReference(c echo.Context) error {
	...
	if result := apiserver_lib.RetryOnSerializationFailure(func() *gorm.DB {
		return h.RequestDB(c).Session(&gorm.Session{FullSaveAssociations: false}).Omit("CreatedAt", "DeletedAt").Save(&updatedAttachedObjectReference)
	}); result.Error != nil {
...
func (h Handler) DeleteAttachedObjectReference(c echo.Context) error {
	...
	// delete object
	if result := apiserver_lib.RetryOnSerializationFailure(func() *gorm.DB {
		return h.RequestDB(c).Delete(&attachedObjectReference)
	}); result.Error != nil {
