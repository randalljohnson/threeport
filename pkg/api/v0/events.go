...
type Event struct {
	...
	Count *uint `gorm:"not null" validate:"required"`

	// Time when this Event was first observed.
	EventTime *time.Time `gorm:"not null" validate:"required"`

	// The time at which the most recent occurrence of this event was recorded.
	LastObservedTime *time.Time `gorm:"not null" validate:"required"`

	// Type of this event (Normal, Warning), new types could be added in the future.
	Type *string `gorm:"not null" validate:"required"`

	// Name of the controller that emitted this Event.
	ReportingController *string `gorm:"not null" validate:"required"`
