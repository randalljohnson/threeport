...
type LogStorageInstance struct {
	...
	LogStorageDefinitionID *uint `validate:"optional,association"`

	// The cluster from which log messages are being aggregated to send to a log
	// back end.
	ClusterID *uint `validate:"optional,association" relationship:"requires;type:KubernetesRuntimeInstance"`
}
