...
import (
	...
	machineruntime "github.com/threeport/threeport/internal/machine-runtime"
...
func main() {
	...
	kvConfig := natsgo.KeyValueConfig{
		Bucket:      machineruntime.LockBucketName,
		Description: machineruntime.LockBucketDescr,
		TTL:         time.Minute * 20,
	}
	kv, err := controller.CreateLockBucketIfNotExists(js, &kvConfig)
	if err != nil {
		log.Error(err, "failed to bind to JetStream key-value locking bucket", "lockBucketName", machineruntime.LockBucketName)
		os.Exit(1)
	}

	// check to ensure machine-runtime stream has been created by API
	machineRuntimeStreamNameFound := false
	for stream := range js.StreamNames() {
		if stream == notif.MachineRuntimeStreamName {
			machineRuntimeStreamNameFound = true
		}
	}
	if !machineRuntimeStreamNameFound {
		log.Error(errors.New("JetStream stream not found"), "failed to find stream with machine-runtime stream name", "machineRuntimeStreamName", notif.MachineRuntimeStreamName)
		os.Exit(1)
	}
	...
	for _, r := range reconcilerConfigs {
		...
		js.AddConsumer(notif.MachineRuntimeStreamName, &natsgo.ConsumerConfig{
		...
		sub, err := js.PullSubscribe(r.NotifSubject, consumer, natsgo.BindStream(notif.MachineRuntimeStreamName))
		if err != nil {
		...
		reconciler := controller.Reconciler{
			...
			EventsRecorder: &event.EventRecorder{
				APIClient:           apiClient,
				APIServer:           *apiServer,
				ReportingController: "MachineRuntimeController",
			},
			JetStreamContext: js,
			KeyValue:         kv,
			Log:              &log,
	...
	log.Info(
		"machine-runtime controller started",
		"version", version.GetVersion(),
		"controllerID", controllerID.String(),
		"NATSConnection", natsConn,
		"lockBucketName", machineruntime.LockBucketName,
	)
	...
	log.Info("machine-runtime controller shutting down")
	os.Exit(0)
}
func showHelpAndExit(exitCode int) {
	fmt.Printf("Usage: threeport-machine-runtime-controller [options]\n")
	fmt.Println("options:")
	flag.PrintDefaults()
	os.Exit(exitCode)
}
