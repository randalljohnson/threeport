...
func (i *KubernetesRuntimeInfraGKE) pulumiProgram() pulumi.RunFunc {
	return func(ctx *pulumi.Context) error {
		// pass credentials for this service account into the Pulumi GCP provider when set
		// so two concurrent stacks do not race a shared credentials environment variable
...
// gcpClientOptions returns options for GCP SDK clients built for this instance.
// It appends WithCredentialsJSON after base when ServiceAccountCredentials is
// set so concurrent calls use that account without a credentials env var; when
// unset it returns base for Application Default Credentials.
...
// tokenSource returns an OAuth2 token source for OAuth scopes from this
// instance's service account JSON when set, else Application Default Credentials.
// A fresh source per call keeps concurrent connects on different accounts from sharing one.
