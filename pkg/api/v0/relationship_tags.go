...
// relationshipFKProvider is implemented by every API type with at least one
...
type relationshipFKProvider interface {
	RelationshipForeignKeys() []relationshipForeignKey
}

// foreignKeysFor returns the relationship FKs of obj, or nil if obj has none.
func foreignKeysFor(obj interface{}) []relationshipForeignKey {
	p, ok := obj.(relationshipFKProvider)
	if !ok {
		return nil
	}
...
func processRelationshipTaggedFieldsBeforeUpdate(tx *gorm.DB, obj interface{}) error {
	for _, fk := range foreignKeysFor(obj) {
		if fk.value == nil {
			continue
		}
