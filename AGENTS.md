...
Every generated file says which kind it is on its own first line, so open the file rather than consulting a list. Boilerplate opens with `// boilerplate generated by 'threeport-sdk gen' - do not edit`. Scaffolding opens with `// scaffolding generated by 'threeport-sdk gen' but will not be regenerated - intended for modification`. A file carrying neither line is hand-written.
