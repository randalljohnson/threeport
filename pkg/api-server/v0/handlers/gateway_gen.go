...
func (h Handler) GetDomainNameDefinitions(c echo.Context) error {
	...
	case pageParams.QueryId == "":
		...
		case true:
			...
			queryId, count, err := h.DispatchGetPaginatedRecords(h.PaginationMode, h.RequestDB(c).Model(&api_v0.DomainNameDefinition{}).Where(&filter), records, queryTable, pageParams)
			if err != nil {
...
func (h Handler) DeleteDomainNameDefinition(c echo.Context) error {
	...
	if len(domainNameDefinition.DomainNameInstances) != 0 {
		blockingChildren := make([]api_lib.FullyQualifiedTypeProvider, 0, len(domainNameDefinition.DomainNameInstances))
		for i := range domainNameDefinition.DomainNameInstances {
			blockingChildren = append(blockingChildren, domainNameDefinition.DomainNameInstances[i])
		}
		return RespondBlockedDelete(
			c,
			h.RequestDB(c),
			api_v0.NewBlockedDeleteErrorFromChildren(&domainNameDefinition, blockingChildren),
		)
	}
...
func (h Handler) GetDomainNameInstances(c echo.Context) error {
	...
	case pageParams.QueryId == "":
		...
		case true:
			// dispatch to the configured pagination strategy to fetch the first page
			queryTable := filter.TableName()
			queryId, count, err := h.DispatchGetPaginatedRecords(h.PaginationMode, h.RequestDB(c).Model(&api_v0.DomainNameInstance{}).Where(&filter), records, queryTable, pageParams)
			if err != nil {
				h.Logger.Error("handler error: error fetching paginated records", zap.Error(err))
				return apiserver_lib.ResponseStatus500(c, pageParams, err, objectType)
			}
...
func (h Handler) GetGatewayDefinitions(c echo.Context) error {
	...
	case pageParams.QueryId == "":
		...
		case true:
			// dispatch to the configured pagination strategy to fetch the first page
			queryTable := filter.TableName()
			queryId, count, err := h.DispatchGetPaginatedRecords(h.PaginationMode, h.RequestDB(c).Model(&api_v0.GatewayDefinition{}).Where(&filter), records, queryTable, pageParams)
			if err != nil {
				h.Logger.Error("handler error: error fetching paginated records", zap.Error(err))
				return apiserver_lib.ResponseStatus500(c, pageParams, err, objectType)
			}
...
func (h Handler) DeleteGatewayDefinition(c echo.Context) error {
	...
	if len(gatewayDefinition.GatewayInstances) != 0 {
		blockingChildren := make([]api_lib.FullyQualifiedTypeProvider, 0, len(gatewayDefinition.GatewayInstances))
		for i := range gatewayDefinition.GatewayInstances {
			blockingChildren = append(blockingChildren, gatewayDefinition.GatewayInstances[i])
		}
		return RespondBlockedDelete(
			c,
			h.RequestDB(c),
			api_v0.NewBlockedDeleteErrorFromChildren(&gatewayDefinition, blockingChildren),
		)
	}
...
func (h Handler) GetGatewayHttpPorts(c echo.Context) error {
	...
	case pageParams.QueryId == "":
		...
		case true:
			// dispatch to the configured pagination strategy to fetch the first page
			queryTable := filter.TableName()
			queryId, count, err := h.DispatchGetPaginatedRecords(h.PaginationMode, h.RequestDB(c).Model(&api_v0.GatewayHttpPort{}).Where(&filter), records, queryTable, pageParams)
			if err != nil {
				h.Logger.Error("handler error: error fetching paginated records", zap.Error(err))
				return apiserver_lib.ResponseStatus500(c, pageParams, err, objectType)
			}
...
func (h Handler) GetGatewayInstances(c echo.Context) error {
	...
	case pageParams.QueryId == "":
		...
		case true:
			// dispatch to the configured pagination strategy to fetch the first page
			queryTable := filter.TableName()
			queryId, count, err := h.DispatchGetPaginatedRecords(h.PaginationMode, h.RequestDB(c).Model(&api_v0.GatewayInstance{}).Where(&filter), records, queryTable, pageParams)
			if err != nil {
				h.Logger.Error("handler error: error fetching paginated records", zap.Error(err))
				return apiserver_lib.ResponseStatus500(c, pageParams, err, objectType)
			}
...
func (h Handler) GetGatewayTcpPorts(c echo.Context) error {
	...
	case pageParams.QueryId == "":
		...
		case true:
			// dispatch to the configured pagination strategy to fetch the first page
			queryTable := filter.TableName()
			queryId, count, err := h.DispatchGetPaginatedRecords(h.PaginationMode, h.RequestDB(c).Model(&api_v0.GatewayTcpPort{}).Where(&filter), records, queryTable, pageParams)
			if err != nil {
				h.Logger.Error("handler error: error fetching paginated records", zap.Error(err))
				return apiserver_lib.ResponseStatus500(c, pageParams, err, objectType)
			}
