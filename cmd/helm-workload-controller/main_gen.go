...
func main() {
	...
	kvConfig := natsgo.KeyValueConfig{
		Bucket:      helmworkload.LockBucketName,
		Description: helmworkload.LockBucketDescr,
		TTL:         time.Minute * 20,
	}
	kv, err := controller.CreateLockBucketIfNotExists(js, &kvConfig)
	if err != nil {
		log.Error(err, "failed to bind to JetStream key-value locking bucket", "lockBucketName", helmworkload.LockBucketName)
		os.Exit(1)
	}
	...
	for _, r := range reconcilerConfigs {
		...
		consumerInfo, err := notifications.EnsureConsumer(js, notif.HelmWorkloadStreamName, consumerConfig)
		if err != nil {
		...
		sub, err := js.PullSubscribe(r.NotifSubject, consumer, natsgo.BindStream(notif.HelmWorkloadStreamName))
		if err != nil {
	...
	log.Info(
		"helm-workload controller started",
		"version", version.GetVersion(),
		"controllerID", controllerID.String(),
		"NATSConnection", natsConn,
		"lockBucketName", helmworkload.LockBucketName,
	)
	...
	log.Info("helm-workload controller shutting down")
	os.Exit(0)
}
func showHelpAndExit(exitCode int) {
	fmt.Printf("Usage: threeport-helm-workload-controller [options]\n")
	fmt.Println("options:")
	flag.PrintDefaults()
	os.Exit(exitCode)
}
