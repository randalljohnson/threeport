...
func (h Handler) AddKubernetesRuntimeDefinition(c echo.Context) error {
	...
	if result := apiserver_lib.RetryOnSerializationFailure(func() *gorm.DB {
		return h.RequestDB(c).Create(&kubernetesRuntimeDefinition)
	}); result.Error != nil {
...
func (h Handler) GetKubernetesRuntimeDefinitions(c echo.Context) error {
	...
	case pageParams.QueryId == "":
		...
		case true:
			...
			queryId, count, err := h.DispatchGetPaginatedRecords(h.PaginationMode, h.RequestDB(c).Model(&api_v0.KubernetesRuntimeDefinition{}).Where(&filter), records, queryTable, pageParams)
			if err != nil {
...
func (h Handler) ReplaceKubernetesRuntimeDefinition(c echo.Context) error {
	...
	if result := apiserver_lib.RetryOnSerializationFailure(func() *gorm.DB {
		return h.RequestDB(c).Session(&gorm.Session{FullSaveAssociations: false}).Omit("CreatedAt", "DeletedAt").Save(&updatedKubernetesRuntimeDefinition)
	}); result.Error != nil {
...
func (h Handler) DeleteKubernetesRuntimeDefinition(c echo.Context) error {
	...
	if kubernetesRuntimeDefinition.DeletionScheduled == nil {
		...
		if result := apiserver_lib.RetryOnSerializationFailure(func() *gorm.DB {
			return h.RequestDB(c).Model(&kubernetesRuntimeDefinition).Updates(&scheduledKubernetesRuntimeDefinition)
		}); result.Error != nil {
		...
		if kubernetesRuntimeDefinition.DeletionConfirmed == nil {
			...
			// from DB
			if result := apiserver_lib.RetryOnSerializationFailure(func() *gorm.DB {
				return h.RequestDB(c).Delete(&kubernetesRuntimeDefinition)
			}); result.Error != nil {
...
func (h Handler) AddKubernetesRuntimeInstance(c echo.Context) error {
	...
	// persist to DB
	if result := apiserver_lib.RetryOnSerializationFailure(func() *gorm.DB {
		return h.RequestDB(c).Create(&kubernetesRuntimeInstance)
	}); result.Error != nil {
		h.Logger.Error("handler error: error creating object", zap.Error(result.Error))
		// check if this is a custom HTTP error with specific status code
		var httpErr *util_v0.HttpError
...
func (h Handler) GetKubernetesRuntimeInstances(c echo.Context) error {
	...
	case pageParams.QueryId == "":
		...
		case true:
			// dispatch to the configured pagination strategy to fetch the first page
			queryTable := filter.TableName()
			queryId, count, err := h.DispatchGetPaginatedRecords(h.PaginationMode, h.RequestDB(c).Model(&api_v0.KubernetesRuntimeInstance{}).Where(&filter), records, queryTable, pageParams)
			if err != nil {
				h.Logger.Error("handler error: error fetching paginated records", zap.Error(err))
				return apiserver_lib.ResponseStatus500(c, pageParams, err, objectType)
			}
...
func (h Handler) ReplaceKubernetesRuntimeInstance(c echo.Context) error {
	...
	if result := apiserver_lib.RetryOnSerializationFailure(func() *gorm.DB {
		return h.RequestDB(c).Session(&gorm.Session{FullSaveAssociations: false}).Omit("CreatedAt", "DeletedAt").Save(&updatedKubernetesRuntimeInstance)
	}); result.Error != nil {
		h.Logger.Error("handler error: error persisting object", zap.Error(result.Error))
		// check if this is a custom HTTP error with specific status code
		var httpErr *util_v0.HttpError
...
func (h Handler) DeleteKubernetesRuntimeInstance(c echo.Context) error {
	...
	if kubernetesRuntimeInstance.DeletionScheduled == nil {
		...
		if result := apiserver_lib.RetryOnSerializationFailure(func() *gorm.DB {
			return h.RequestDB(c).Model(&kubernetesRuntimeInstance).Updates(&scheduledKubernetesRuntimeInstance)
		}); result.Error != nil {
			h.Logger.Error("handler error: error creating scheduled deletion", zap.Error(result.Error))
			return apiserver_lib.ResponseStatus500(c, nil, result.Error, objectType)
		}
		...
		if kubernetesRuntimeInstance.DeletionConfirmed == nil {
			...
			// from DB
			if result := apiserver_lib.RetryOnSerializationFailure(func() *gorm.DB {
				return h.RequestDB(c).Delete(&kubernetesRuntimeInstance)
			}); result.Error != nil {
				h.Logger.Error("handler error: error deleting object", zap.Error(result.Error))
				// surface BlockedDeleteError from gorm hook - backstop in case an attached object reference was created after the pre-check
				var blockedErr *api_v0.BlockedDeleteError
