...
// @Summary GetGcpNetworkVersions gets the supported versions for the gcp network API.
// @Description Get the supported API versions for gcp networks.
// @ID gcpNetwork-get-versions
// @Produce json
// @Success 200 {object} apiserver_lib.ApiObjectVersions "OK"
// @Router /gcp-networks/versions [GET]
func (h Handler) GetGcpNetworkVersions(c echo.Context) error {
	return c.JSON(http.StatusOK, apiserver_lib.ObjectVersions[string(api_v0.ObjectTypeGcpNetwork)])
}

// @Summary adds a new gcp network.
// @Description Add a new gcp network to the Threeport database.
// @ID add-v0-gcpNetwork
// @Accept json
// @Produce json
// @Param gcpNetwork body api_v0.GcpNetwork true "GcpNetwork object"
// @Success 201 {object} v0.Response "Created"
// @Failure 400 {object} v0.Response "Bad Request"
// @Failure 500 {object} v0.Response "Internal Server Error"
// @Router /v0/gcp-networks [POST]
func (h Handler) AddGcpNetwork(c echo.Context) error {
	objectType := api_v0.ObjectTypeGcpNetwork
	var gcpNetwork api_v0.GcpNetwork

	// check for empty payload, unsupported fields, GORM Model fields, optional associations, etc.
	if id, err := apiserver_lib.PayloadCheck(c, false, false, objectType, gcpNetwork); err != nil {
		h.Logger.Error("handler error: error performing payload check", zap.Error(err))
		return apiserver_lib.ResponseStatusErr(id, c, nil, errors.New(err.Error()), objectType)
	}

	if err := c.Bind(&gcpNetwork); err != nil {
		h.Logger.Error("handler error: error binding object", zap.Error(err))
		return apiserver_lib.ResponseStatus500(c, nil, err, objectType)
	}

	// check for missing required fields
	if id, err := apiserver_lib.ValidateBoundData(c, gcpNetwork, objectType); err != nil {
		h.Logger.Error("handler error: error validating bound data", zap.Error(err))
		return apiserver_lib.ResponseStatusErr(id, c, nil, errors.New(err.Error()), objectType)
	}

	// check for duplicate names
	var existingGcpNetwork api_v0.GcpNetwork
	nameUsed := true
	result := h.RequestDB(c).Where("name = ?", gcpNetwork.Name).First(&existingGcpNetwork)
	if result.Error != nil {
		if errors.Is(result.Error, gorm.ErrRecordNotFound) {
			nameUsed = false
		} else {
	...
	if result := apiserver_lib.RetryOnSerializationFailure(func() *gorm.DB {
		return h.RequestDB(c).Create(&gcpNetwork)
	}); result.Error != nil {
		h.Logger.Error("handler error: error creating object", zap.Error(result.Error))
		// check if this is a custom HTTP error with specific status code
		var httpErr *util_v0.HttpError
	...
	// notify controller if reconciliation is required
	if !*gcpNetwork.Reconciled {
		notifPayload, err := gcpNetwork.NotificationPayload(
			notifications.NotificationOperationCreated,
			false,
			time.Now().Unix(),
		)
		...
		h.JS.Publish(notif.GcpNetworkCreateSubject, *notifPayload)
	}
...
// @Summary gets all gcp networks.
// @Description Get all gcp networks from the Threeport database.
// @ID get-v0-gcpNetworks
// @Accept json
// @Produce json
// @Param name query string false "gcp network search by name"
// @Success 200 {object} v0.Response "OK"
// @Failure 400 {object} v0.Response "Bad Request"
// @Failure 500 {object} v0.Response "Internal Server Error"
// @Router /v0/gcp-networks [GET]
func (h Handler) GetGcpNetworks(c echo.Context) error {
	objectType := api_v0.ObjectTypeGcpNetwork
	...
	case pageParams.QueryId == "":
		...
		// count total number of objects
		var totalCount int64
		if result := h.RequestDB(c).Model(&api_v0.GcpNetwork{}).Where(&filter).Count(&totalCount); result.Error != nil {
			h.Logger.Error("handler error: error counting objects", zap.Error(result.Error))
			return apiserver_lib.ResponseStatus500(c, pageParams, result.Error, objectType)
		}
...
// @Summary gets a gcp network.
// @Description Get a particular gcp network from the database.
// @ID get-v0-gcpNetwork
// @Accept json
// @Produce json
// @Param id path int true "ID"
// @Success 200 {object} v0.Response "OK"
// @Failure 404 {object} v0.Response "Not Found"
// @Failure 500 {object} v0.Response "Internal Server Error"
// @Router /v0/gcp-networks/{id} [GET]
func (h Handler) GetGcpNetwork(c echo.Context) error {
	objectType := api_v0.ObjectTypeGcpNetwork
	gcpNetworkID := c.Param("id")
	var gcpNetwork api_v0.GcpNetwork
	if result := h.RequestDB(c).
		First(&gcpNetwork, gcpNetworkID); result.Error != nil {
		if errors.Is(result.Error, gorm.ErrRecordNotFound) {
			return apiserver_lib.ResponseStatus404(c, nil, result.Error, objectType)
		}
		h.Logger.Error("handler error: error finding object", zap.Error(result.Error))
...
// @Summary updates specific fields for an existing gcp network.
// @Description Update a gcp network in the database.  Provide one or more fields to update.
// @Description Note: This API endpint is for updating gcp network objects only.
// @Description Request bodies that include related objects will be accepted, however
// @Description the related objects will not be changed.  Call the patch or put method for
// @Description each particular existing object to change them.
// @ID update-v0-gcpNetwork
// @Accept json
// @Produce json
// @Param id path int true "ID"
// @Param gcpNetwork body api_v0.GcpNetwork true "GcpNetwork object"
...
// @Failure 404 {object} v0.Response "Not Found"
// @Failure 500 {object} v0.Response "Internal Server Error"
// @Router /v0/gcp-networks/{id} [PATCH]
func (h Handler) UpdateGcpNetwork(c echo.Context) error {
	objectType := api_v0.ObjectTypeGcpNetwork
	gcpNetworkID := c.Param("id")
	var existingGcpNetwork api_v0.GcpNetwork
	if result := h.RequestDB(c).First(&existingGcpNetwork, gcpNetworkID); result.Error != nil {
		if errors.Is(result.Error, gorm.ErrRecordNotFound) {
			return apiserver_lib.ResponseStatus404(c, nil, result.Error, objectType)
		}
		h.Logger.Error("handler error: error finding object", zap.Error(result.Error))
	...
	// check for empty payload, invalid or unsupported fields, optional associations, etc.
	if id, err := apiserver_lib.PayloadCheck(c, false, true, objectType, existingGcpNetwork); err != nil {
		h.Logger.Error("handler error: error performing payload check", zap.Error(err))
		return apiserver_lib.ResponseStatusErr(id, c, nil, errors.New(err.Error()), objectType)
	}

	// bind payload
	var updatedGcpNetwork api_v0.GcpNetwork
	if err := c.Bind(&updatedGcpNetwork); err != nil {
		h.Logger.Error("handler error: error binding payload", zap.Error(err))
		return apiserver_lib.ResponseStatus500(c, nil, err, objectType)
	}
	...
	prevReconciliation := existingGcpNetwork.Reconciliation

	// update object in database
	if result := apiserver_lib.RetryOnSerializationFailure(func() *gorm.DB {
		return h.RequestDB(c).Model(&existingGcpNetwork).Updates(&updatedGcpNetwork)
	}); result.Error != nil {
		h.Logger.Error("handler error: error updating object", zap.Error(result.Error))
		// check if this is a custom HTTP error with specific status code
		var httpErr *util_v0.HttpError
	...
	if existingGcpNetwork.Reconciled != nil && !*existingGcpNetwork.Reconciled && api_v0.ReconciliationStateChanged(prevReconciliation, existingGcpNetwork.Reconciliation) {
		notifPayload, err := existingGcpNetwork.NotificationPayload(
			notifications.NotificationOperationUpdated,
			false,
			time.Now().Unix(),
		)
		...
		h.JS.Publish(notif.GcpNetworkUpdateSubject, *notifPayload)
	}
...
// @Summary updates an existing gcp network by replacing the entire object.
// @Description Replace a gcp network in the database.  All required fields must be provided.
// @Description If any optional fields are not provided, they will be null post-update.
// @Description Note: This API endpint is for updating gcp network objects only.
// @Description Request bodies that include related objects will be accepted, however
// @Description the related objects will not be changed.  Call the patch or put method for
// @Description each particular existing object to change them.
// @ID replace-v0-gcpNetwork
// @Accept json
// @Produce json
// @Param id path int true "ID"
// @Param gcpNetwork body api_v0.GcpNetwork true "GcpNetwork object"
...
// @Failure 404 {object} v0.Response "Not Found"
// @Failure 500 {object} v0.Response "Internal Server Error"
// @Router /v0/gcp-networks/{id} [PUT]
func (h Handler) ReplaceGcpNetwork(c echo.Context) error {
	objectType := api_v0.ObjectTypeGcpNetwork
	gcpNetworkID := c.Param("id")
	var existingGcpNetwork api_v0.GcpNetwork
	if result := h.RequestDB(c).First(&existingGcpNetwork, gcpNetworkID); result.Error != nil {
	...
	// check for missing required fields
	if id, err := apiserver_lib.ValidateBoundData(c, updatedGcpNetwork, objectType); err != nil {
		h.Logger.Error("handler error: error validating bound data", zap.Error(err))
		return apiserver_lib.ResponseStatusErr(id, c, nil, errors.New(err.Error()), objectType)
	}

	// persist provided data
	updatedGcpNetwork.ID = existingGcpNetwork.ID
	if result := apiserver_lib.RetryOnSerializationFailure(func() *gorm.DB {
		return h.RequestDB(c).Session(&gorm.Session{FullSaveAssociations: false}).Omit("CreatedAt", "DeletedAt").Save(&updatedGcpNetwork)
	}); result.Error != nil {
		h.Logger.Error("handler error: error persisting object", zap.Error(result.Error))
		// check if this is a custom HTTP error with specific status code
		var httpErr *util_v0.HttpError
...
// @Summary deletes a gcp network.
// @Description Delete a gcp network by ID from the database.
// @ID delete-v0-gcpNetwork
// @Accept json
// @Produce json
// @Param id path int true "ID"
// @Success 200 {object} v0.Response "OK"
...
// @Failure 409 {object} v0.Response "Conflict"
// @Failure 500 {object} v0.Response "Internal Server Error"
// @Router /v0/gcp-networks/{id} [DELETE]
func (h Handler) DeleteGcpNetwork(c echo.Context) error {
	objectType := api_v0.ObjectTypeGcpNetwork
	gcpNetworkID := c.Param("id")
	var gcpNetwork api_v0.GcpNetwork
	if result := h.RequestDB(c).First(&gcpNetwork, gcpNetworkID); result.Error != nil {
		if errors.Is(result.Error, gorm.ErrRecordNotFound) {
			return apiserver_lib.ResponseStatus404(c, nil, result.Error, objectType)
		}
		h.Logger.Error("handler error: error finding object", zap.Error(result.Error))
	...
	// pre-check synchronously so the client sees the 409 - without this, reconciled types only surface the block to the reconciler
	if checkErr := api_v0.CheckBlockingAttachedObjectReferences(h.RequestDB(c), &gcpNetwork); checkErr != nil {
		var blockedErr *api_v0.BlockedDeleteError
		if errors.As(checkErr, &blockedErr) {
			return RespondBlockedDelete(
				c,
	...
	// if scheduled and reconciled, delete object from DB
	// if scheduled but not reconciled, return 409 (controller is working on it)
	if gcpNetwork.DeletionScheduled == nil {
		// schedule for deletion
		reconciled := false
		timestamp := time.Now().UTC()
		scheduledGcpNetwork := api_v0.GcpNetwork{
			Reconciliation: api_v0.Reconciliation{
				DeletionScheduled: &timestamp,
				Reconciled:        &reconciled,
			}}
		if result := apiserver_lib.RetryOnSerializationFailure(func() *gorm.DB {
			return h.RequestDB(c).Model(&gcpNetwork).Updates(&scheduledGcpNetwork)
		}); result.Error != nil {
			h.Logger.Error("handler error: error creating scheduled deletion", zap.Error(result.Error))
			return apiserver_lib.ResponseStatus500(c, nil, result.Error, objectType)
		}
		...
		h.JS.Publish(notif.GcpNetworkDeleteSubject, *notifPayload)
	} else {
		if gcpNetwork.DeletionConfirmed == nil {
			// if deletion scheduled but not reconciled, return 409 - deletion
			// already underway
			return apiserver_lib.ResponseStatus409(c, nil, errors.New(fmt.Sprintf(
				"object with ID %d already being deleted",
			...
			// from DB
			if result := apiserver_lib.RetryOnSerializationFailure(func() *gorm.DB {
				return h.RequestDB(c).Delete(&gcpNetwork)
			}); result.Error != nil {
				h.Logger.Error("handler error: error deleting object", zap.Error(result.Error))
				// surface BlockedDeleteError from gorm hook - backstop in case an attached object reference was created after the pre-check
				var blockedErr *api_v0.BlockedDeleteError
