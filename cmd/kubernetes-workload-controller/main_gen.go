...
import (
	...
	kubernetesworkload "github.com/threeport/threeport/internal/kubernetes-workload"
...
func main() {
	...
	kvConfig := natsgo.KeyValueConfig{
		Bucket:      kubernetesworkload.LockBucketName,
		Description: kubernetesworkload.LockBucketDescr,
		TTL:         time.Minute * 20,
	}
	kv, err := controller.CreateLockBucketIfNotExists(js, &kvConfig)
	if err != nil {
		log.Error(err, "failed to bind to JetStream key-value locking bucket", "lockBucketName", kubernetesworkload.LockBucketName)
		os.Exit(1)
	}
	...
	for _, r := range reconcilerConfigs {
		...
		_, err = js.AddConsumer(notif.KubernetesWorkloadStreamName, &natsgo.ConsumerConfig{
		...
		reconciler := controller.Reconciler{
			...
			EventsRecorder: &event.EventRecorder{
				APIClient:           apiClient,
				APIServer:           *apiServer,
				ReportingController: "KubernetesWorkloadController",
			},
			JetStreamContext: js,
			KeyValue:         kv,
			Log:              &log,
	...
	log.Info(
		"kubernetes-workload controller started",
		"version", version.GetVersion(),
		"controllerID", controllerID.String(),
		"NATSEndpoint", natsEndpoint,
		"lockBucketName", kubernetesworkload.LockBucketName,
	)
	...
	log.Info("kubernetes-workload controller shutting down")
	os.Exit(0)
}
func showHelpAndExit(exitCode int) {
	fmt.Printf("Usage: threeport-kubernetes-workload-controller [options]\n")
	fmt.Println("options:")
	flag.PrintDefaults()
	os.Exit(exitCode)
}
