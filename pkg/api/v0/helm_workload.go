...
type HelmWorkloadDefinition struct {
	...
	ChartVersion *string `query:"chartversion" validate:"optional"`
	...
	ValuesDocument *string `validate:"optional"`
	...
	// helm chart.
	AdditionalResources *datatypes.JSONSlice[datatypes.JSON] `validate:"optional"`
}
...
type HelmWorkloadInstance struct {
	...
	ReleaseNamespace *string `query:"releasenamespace" validate:"optional"`

	// The definition used to configure the workload instance.
	HelmWorkloadDefinitionID *uint `query:"helmworkloaddefinitionid" gorm:"not null" validate:"required" relationship:"requires"`
