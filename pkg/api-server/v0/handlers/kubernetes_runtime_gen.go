...
func (h Handler) ReplaceKubernetesRuntimeDefinition(c echo.Context) error {
	...
	if err := crdbgorm.ExecuteTx(
		c.Request().Context(), h.DB, nil,
		func(tx *gorm.DB) error {
			return tx.Scopes(apiserver_lib.QueryScopes(c)...).Session(&gorm.Session{FullSaveAssociations: false}).Omit("CreatedAt", "DeletedAt").Save(&updatedKubernetesRuntimeDefinition).Error
		},
	); err != nil {
...
func (h Handler) DeleteKubernetesRuntimeDefinition(c echo.Context) error {
	...
	if kubernetesRuntimeDefinition.DeletionScheduled == nil {
		...
		if err := crdbgorm.ExecuteTx(
			c.Request().Context(), h.DB, nil,
			func(tx *gorm.DB) error {
				return tx.Scopes(apiserver_lib.QueryScopes(c)...).Model(&kubernetesRuntimeDefinition).Updates(&scheduledKubernetesRuntimeDefinition).Error
			},
		); err != nil {
		...
		if kubernetesRuntimeDefinition.DeletionConfirmed == nil {
			...
			if err := crdbgorm.ExecuteTx(
				c.Request().Context(), h.DB, nil,
				func(tx *gorm.DB) error {
					return tx.Scopes(apiserver_lib.QueryScopes(c)...).Delete(&kubernetesRuntimeDefinition).Error
				},
			); err != nil {
...
func (h Handler) ReplaceKubernetesRuntimeInstance(c echo.Context) error {
	...
	if err := crdbgorm.ExecuteTx(
		c.Request().Context(), h.DB, nil,
		func(tx *gorm.DB) error {
			return tx.Scopes(apiserver_lib.QueryScopes(c)...).Session(&gorm.Session{FullSaveAssociations: false}).Omit("CreatedAt", "DeletedAt").Save(&updatedKubernetesRuntimeInstance).Error
		},
	); err != nil {
		h.Logger.Error("handler error: error persisting object", zap.Error(err))
		// check if this is a custom HTTP error with specific status code
...
func (h Handler) DeleteKubernetesRuntimeInstance(c echo.Context) error {
	...
	if kubernetesRuntimeInstance.DeletionScheduled == nil {
		...
		if err := crdbgorm.ExecuteTx(
			c.Request().Context(), h.DB, nil,
			func(tx *gorm.DB) error {
				return tx.Scopes(apiserver_lib.QueryScopes(c)...).Model(&kubernetesRuntimeInstance).Updates(&scheduledKubernetesRuntimeInstance).Error
			},
		); err != nil {
			h.Logger.Error("handler error: error creating scheduled deletion", zap.Error(err))
			return apiserver_lib.ResponseStatus500(c, nil, err, objectType)
		...
		if kubernetesRuntimeInstance.DeletionConfirmed == nil {
			...
			if err := crdbgorm.ExecuteTx(
				c.Request().Context(), h.DB, nil,
				func(tx *gorm.DB) error {
					return tx.Scopes(apiserver_lib.QueryScopes(c)...).Delete(&kubernetesRuntimeInstance).Error
				},
			); err != nil {
				h.Logger.Error("handler error: error deleting object", zap.Error(err))
				// surface BlockedDeleteError from gorm hook - backstop in case an attached object reference was created after the pre-check
