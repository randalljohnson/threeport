...
const mainHelperEnvVar = "GATEWAY_CTRL_RUN_MAIN"
...
func TestMain(m *testing.M) {
	...
	if argv, ok := os.LookupEnv(mainHelperEnvVar); ok {
		...
		os.Args = append([]string{"gateway-controller"}, strings.Fields(argv)...)
		main()
...
func TestMainHelpFlagPrintsFlagDefaults(t *testing.T) {
	...
	if !strings.HasPrefix(stdout, "Usage: threeport-gateway-controller [options]") {
	...
	if !strings.Contains(stderr, "-gateway-definition-concurrent-reconciles") {
		t.Fatalf("stderr missing -gateway-definition-concurrent-reconciles flag: %q", stderr)
	}
