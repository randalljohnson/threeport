...
func (h Handler) AddMachineRuntimeDefinition(c echo.Context) error {
	...
	if err := apiserver_lib.AfterCommitCreate(h.DB, &machineRuntimeDefinition); err != nil {
...
func (h Handler) ReplaceMachineRuntimeDefinition(c echo.Context) error {
	...
	if result := h.Write(c, func(db *gorm.DB) *gorm.DB {
		return db.Session(&gorm.Session{FullSaveAssociations: false}).Omit("CreatedAt", "DeletedAt").Save(&updatedMachineRuntimeDefinition)
	}); result.Error != nil {
...
func (h Handler) DeleteMachineRuntimeDefinition(c echo.Context) error {
	...
	if len(machineRuntimeDefinition.MachineRuntimeInstances) != 0 {
		err := errors.New("machine runtime definition has related machine runtime instances - " + api_v0.ErrMsgDeleteBlocked)
	...
	if err := apiserver_lib.AfterCommitDelete(h.DB, &machineRuntimeDefinition); err != nil {
		h.Logger.Error("handler error: error reconciling process state after commit", zap.Error(err))
		return apiserver_lib.ResponseStatus500(c, nil, err, fullyQualifiedType)
	}
...
func (h Handler) AddMachineRuntimeInstance(c echo.Context) error {
	...
	if result := h.Write(c, func(db *gorm.DB) *gorm.DB {
		...
		return apiserver_lib.RespondWriteError(
			...
			h.Logger,
			result.Error,
			new(api_v0.MachineRuntimeInstance),
			fullyQualifiedType,
		)
	}
	...
	// rely on it. A persist hook cannot do this: it runs inside the
	// transaction, so it would act on a write that may never commit.
	if err := apiserver_lib.AfterCommitCreate(h.DB, &machineRuntimeInstance); err != nil {
		h.Logger.Error("handler error: error reconciling process state after commit", zap.Error(err))
		return apiserver_lib.ResponseStatus500(c, nil, err, fullyQualifiedType)
	}
...
func (h Handler) UpdateMachineRuntimeInstance(c echo.Context) error {
	...
	// update object in database
	if result := h.Write(c, func(db *gorm.DB) *gorm.DB {
		return db.Model(&existingMachineRuntimeInstance).Updates(&updatedMachineRuntimeInstance)
	}); result.Error != nil {
		h.Logger.Error("handler error: error updating object", zap.Error(result.Error))
		// check if this is a custom HTTP error with specific status code
		var httpErr *util_v0.HttpError
...
func (h Handler) ReplaceMachineRuntimeInstance(c echo.Context) error {
	...
	if result := h.Write(c, func(db *gorm.DB) *gorm.DB {
		return db.Session(&gorm.Session{FullSaveAssociations: false}).Omit("CreatedAt", "DeletedAt").Save(&updatedMachineRuntimeInstance)
	}); result.Error != nil {
		h.Logger.Error("handler error: error persisting object", zap.Error(result.Error))
		// check if this is a custom HTTP error with specific status code
		var httpErr *util_v0.HttpError
...
func (h Handler) DeleteMachineRuntimeInstance(c echo.Context) error {
	...
	if machineRuntimeInstance.DeletionScheduled == nil {
		...
		if result := h.Write(c, func(db *gorm.DB) *gorm.DB {
			return db.Model(&machineRuntimeInstance).Updates(&scheduledMachineRuntimeInstance)
		}); result.Error != nil {
			h.Logger.Error("handler error: error creating scheduled deletion", zap.Error(result.Error))
			return apiserver_lib.ResponseStatus500(c, nil, result.Error, fullyQualifiedType)
		}
		...
		if machineRuntimeInstance.DeletionConfirmed == nil {
			...
			// from DB
			if result := h.Write(c, func(db *gorm.DB) *gorm.DB {
				return db.Delete(&machineRuntimeInstance)
			}); result.Error != nil {
				h.Logger.Error("handler error: error deleting object", zap.Error(result.Error))
				// surface BlockedDeleteError from gorm hook - backstop in case an attached object reference was created after the pre-check
				var blockedErr *api_v0.BlockedDeleteError
	...
	// row before answering. A persist hook cannot do this: a rollback
	// would have dropped state for a row that survived.
	if err := apiserver_lib.AfterCommitDelete(h.DB, &machineRuntimeInstance); err != nil {
		h.Logger.Error("handler error: error reconciling process state after commit", zap.Error(err))
		return apiserver_lib.ResponseStatus500(c, nil, err, fullyQualifiedType)
	}
