...
import (
	...
	oci "github.com/threeport/threeport/internal/oci"
...
func main() {
	...
	kvConfig := natsgo.KeyValueConfig{
		Bucket:      oci.LockBucketName,
		Description: oci.LockBucketDescr,
		TTL:         time.Minute * 20,
	}
	kv, err := controller.CreateLockBucketIfNotExists(js, &kvConfig)
	if err != nil {
		log.Error(err, "failed to bind to JetStream key-value locking bucket", "lockBucketName", oci.LockBucketName)
		os.Exit(1)
	}
	...
	for _, r := range reconcilerConfigs {
		...
		consumerInfo, err := notifications.EnsureConsumer(js, notif.OciStreamName, consumerConfig)
		if err != nil {
		...
		sub, err := js.PullSubscribe(r.NotifSubject, consumer, natsgo.BindStream(notif.OciStreamName))
		if err != nil {
		...
		reconciler := controller.Reconciler{
			...
			EventsRecorder: &event.EventRecorder{
				APIClient:           apiClient,
				APIServer:           *apiServer,
				ReportingController: "OciController",
			},
			JetStreamContext: js,
			KeyValue:         kv,
			Log:              &log,
	...
	log.Info("oci controller shutting down")
	os.Exit(0)
}
func showHelpAndExit(exitCode int) {
	fmt.Printf("Usage: threeport-oci-controller [options]\n")
	fmt.Println("options:")
	flag.PrintDefaults()
	os.Exit(exitCode)
}
