...
func (h Handler) GetHelmWorkloadDefinitions(c echo.Context) error {
	...
	case pageParams.QueryId == "":
		...
		case true:
			...
			queryId, count, err := h.DispatchGetPaginatedRecords(h.PaginationMode, h.RequestDB(c).Model(&api_v0.HelmWorkloadDefinition{}).Where(&filter), records, queryTable, pageParams)
			if err != nil {
...
func (h Handler) DeleteHelmWorkloadDefinition(c echo.Context) error {
	...
	if len(helmWorkloadDefinition.HelmWorkloadInstances) != 0 {
		blockingChildren := make([]api_lib.FullyQualifiedTypeProvider, 0, len(helmWorkloadDefinition.HelmWorkloadInstances))
		for i := range helmWorkloadDefinition.HelmWorkloadInstances {
			blockingChildren = append(blockingChildren, helmWorkloadDefinition.HelmWorkloadInstances[i])
		}
		return RespondBlockedDelete(
			c,
			h.RequestDB(c),
			api_v0.NewBlockedDeleteErrorFromChildren(&helmWorkloadDefinition, blockingChildren),
		)
	}
...
func (h Handler) GetHelmWorkloadInstances(c echo.Context) error {
	...
	case pageParams.QueryId == "":
		...
		case true:
			// dispatch to the configured pagination strategy to fetch the first page
			queryTable := filter.TableName()
			queryId, count, err := h.DispatchGetPaginatedRecords(h.PaginationMode, h.RequestDB(c).Model(&api_v0.HelmWorkloadInstance{}).Where(&filter), records, queryTable, pageParams)
			if err != nil {
				h.Logger.Error("handler error: error fetching paginated records", zap.Error(err))
				return apiserver_lib.ResponseStatus500(c, pageParams, err, objectType)
			}
