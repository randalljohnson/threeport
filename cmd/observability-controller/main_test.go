...
const helperEnvVar = "OBSERVABILITY_CTRL_HELP_EXIT_CODE"
...
const mainHelperEnvVar = "OBSERVABILITY_CTRL_RUN_MAIN"
...
func TestMain(m *testing.M) {
	...
	if argv, ok := os.LookupEnv(mainHelperEnvVar); ok {
		...
		os.Args = append([]string{"observability-controller"}, strings.Fields(argv)...)
		main()
...
func TestShowHelpAndExitPrintsUsage(t *testing.T) {
	...
	wantPrefix := "Usage: threeport-observability-controller [options]"
...
func TestMainHelpFlagPrintsFlagDefaults(t *testing.T) {
	...
	if !strings.HasPrefix(stdout, "Usage: threeport-observability-controller [options]") {
	...
	if !strings.Contains(stderr, "-observability-stack-definition-concurrent-reconciles") {
		t.Fatalf("stderr missing -observability-stack-definition-concurrent-reconciles flag: %q", stderr)
	}
	...
	// verify a metrics-family flag shows so every reconciler family registers.
	if !strings.Contains(stderr, "-metrics-definition-concurrent-reconciles") {
		t.Fatalf("stderr missing -metrics-definition-concurrent-reconciles flag: %q", stderr)
	}
	// verify a logging-family flag shows so every reconciler family registers.
	if !strings.Contains(stderr, "-logging-definition-concurrent-reconciles") {
		t.Fatalf("stderr missing -logging-definition-concurrent-reconciles flag: %q", stderr)
	}
}
