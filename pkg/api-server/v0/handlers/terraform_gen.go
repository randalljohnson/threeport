...
func (h Handler) ReplaceTerraformDefinition(c echo.Context) error {
	...
	if err := crdbgorm.ExecuteTx(
		c.Request().Context(), h.DB, nil,
		func(tx *gorm.DB) error {
			return tx.Scopes(apiserver_lib.QueryScopes(c)...).Session(&gorm.Session{FullSaveAssociations: false}).Omit("CreatedAt", "DeletedAt").Save(&updatedTerraformDefinition).Error
		},
	); err != nil {
...
func (h Handler) DeleteTerraformDefinition(c echo.Context) error {
	...
	if terraformDefinition.DeletionScheduled == nil {
		...
		if err := crdbgorm.ExecuteTx(
			c.Request().Context(), h.DB, nil,
			func(tx *gorm.DB) error {
				return tx.Scopes(apiserver_lib.QueryScopes(c)...).Model(&terraformDefinition).Updates(&scheduledTerraformDefinition).Error
			},
		); err != nil {
		...
		if terraformDefinition.DeletionConfirmed == nil {
			...
			if err := crdbgorm.ExecuteTx(
				c.Request().Context(), h.DB, nil,
				func(tx *gorm.DB) error {
					return tx.Scopes(apiserver_lib.QueryScopes(c)...).Delete(&terraformDefinition).Error
				},
			); err != nil {
...
func (h Handler) ReplaceTerraformInstance(c echo.Context) error {
	...
	if err := crdbgorm.ExecuteTx(
		c.Request().Context(), h.DB, nil,
		func(tx *gorm.DB) error {
			return tx.Scopes(apiserver_lib.QueryScopes(c)...).Session(&gorm.Session{FullSaveAssociations: false}).Omit("CreatedAt", "DeletedAt").Save(&updatedTerraformInstance).Error
		},
	); err != nil {
		h.Logger.Error("handler error: error persisting object", zap.Error(err))
		// check if this is a custom HTTP error with specific status code
...
func (h Handler) DeleteTerraformInstance(c echo.Context) error {
	...
	if terraformInstance.DeletionScheduled == nil {
		...
		if err := crdbgorm.ExecuteTx(
			c.Request().Context(), h.DB, nil,
			func(tx *gorm.DB) error {
				return tx.Scopes(apiserver_lib.QueryScopes(c)...).Model(&terraformInstance).Updates(&scheduledTerraformInstance).Error
			},
		); err != nil {
			h.Logger.Error("handler error: error creating scheduled deletion", zap.Error(err))
			return apiserver_lib.ResponseStatus500(c, nil, err, objectType)
		...
		if terraformInstance.DeletionConfirmed == nil {
			...
			if err := crdbgorm.ExecuteTx(
				c.Request().Context(), h.DB, nil,
				func(tx *gorm.DB) error {
					return tx.Scopes(apiserver_lib.QueryScopes(c)...).Delete(&terraformInstance).Error
				},
			); err != nil {
				h.Logger.Error("handler error: error deleting object", zap.Error(err))
				// surface BlockedDeleteError from gorm hook - backstop in case an attached object reference was created after the pre-check
